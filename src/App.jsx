...
const Website = () => {
  ...
  // State pour le Quiz
  const [quizStep, setQuizStep] = useState(0); // 0 = intro, 1-4 = questions, 5 = résultat
  ...
  const speciesData = {
    ...
    syrien: {
      name: "Hamster Syrien",
      image: "/hamster_syrien_result2_1767561359036.png",
    ...
    roborovski: {
      ...
      image: "https://images.unsplash.com/photo-1591873386927-ddf23b2a1795?auto=format&fit=crop&q=80&w=800",
    ...
    chinois: {
      ...
      image: "https://images.unsplash.com/photo-1628009368231-7bb7cfcb0291?auto=format&fit=crop&q=80&w=800",
  ...
  const faqData = [
    ...
    {
      question: "Quelle alimentation pour mon hamster ?",
      answer: (
        <div className="space-y-4">
          <p>Un hamster russe a besoin d'un régime équilibré composé de :</p>
          <ul className="list-disc ml-6 space-y-2 text-gray-700">
            <li><strong>Mélange de graines :</strong> Privilégiez les mélanges spécifiques pour hamsters nains (plus petites graines).</li>
            <li><strong>Protéines :</strong> Quelques vers de farine séchés ou un peu de blanc de poulet cuit une fois par semaine.</li>
            <li><strong>Végétaux :</strong> Endive, concombre, brocoli (en petites quantités pour éviter les diarrhées).</li>
          </ul>
          <p className="text-sm italic text-gray-500">🚫 Évitez absolument le sucre (fruits trop sucrés) : les hamsters nains sont sujets au diabète.</p>
        </div>
      )
    },
    {
      question: "Quel habitat est le plus adapté ?",
      answer: (
        <div className="space-y-4">
          <p>L'espace minimum recommandé est de <strong>80x40cm</strong> au sol. Nous conseillons :</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm">
              <h4 className="font-bold text-gray-900 mb-1 flex items-center">
                <Maximize className="w-4 h-4 mr-2 text-amber-600" />
                La cage à barreaux
              </h4>
              <p className="text-sm text-gray-600">Offre une excellente aération et permet au hamster de grimper.</p>
            </div>
            <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm">
              <h4 className="font-bold text-gray-900 mb-1 flex items-center">
                <Minimize className="w-4 h-4 mr-2 text-amber-600" />
                Le bac plastique / Terrarium
              </h4>
              <p className="text-sm text-gray-600">Évite les projections de litière et les courants d'air.</p>
            </div>
          </div>
          <p className="text-sm font-bold text-amber-800">Élément crucial : Une roue pleine d'un diamètre de 20cm minimum pour préserver sa colonne vertébrale.</p>
        </div>
      )
    },
    {
      question: "Comment apprivoiser mon nouveau compagnon ?",
      answer: (
        <div className="space-y-4">
          <p>La patience est la clé. Voici nos 3 étapes d'or :</p>
          <ol className="list-decimal ml-6 space-y-3 text-gray-700">
            <li><strong>L'adaptation (J+2) :</strong> Laissez-le tranquille 48h dans son nouvel habitat pour qu'il prenne ses marques.</li>
            <li><strong>L'odeur (J+4) :</strong> Proposez-lui votre main (paume ouverte) avec une petite graine, sans chercher à le saisir.</li>
            <li><strong>Le contact (J+7) :</strong> Une fois qu'il monte de lui-même, soulevez-le doucement à quelques centimètres du sol.</li>
          </ol>
        </div>
      )
    },
    {
      question: "Détails sur l'adoption (Prix, Âge, Lieu)",
      answer: (
    ...
    {
      question: "Quel Hamster Choisir ? (Test interactif)",
      answer: (
        <div className="space-y-6 text-gray-700 bg-white p-6 rounded-2xl border border-amber-100 shadow-inner">
          {quizStep === 0 && (
            <div className="text-center animate-in fade-in zoom-in duration-500">
              <div className="w-20 h-20 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-6">
                <HelpCircle className="w-10 h-10 text-amber-600" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-4">Trouvons votre compagnon idéal</h3>
              <p className="text-sm text-gray-600 mb-8 max-w-md mx-auto">
                Chaque espèce a son caractère. Répondez à 4 questions rapides pour savoir quel hamster correspond à votre foyer.
              </p>
              <button
          ...
          {quizStep === 1 && (
            <div className="animate-in slide-in-from-right duration-500">
              <span className="text-[10px] uppercase tracking-widest text-amber-600 font-bold mb-2 block">Question 1/4</span>
              <h3 className="text-lg font-bold mb-6">Quel est votre niveau d'expérience ?</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  { id: 'debutant', label: 'Débutant', icon: '🌱' },
                  { id: 'moyen', label: 'Intermédiaire', icon: '🌿' },
                  { id: 'expert', label: 'Expert', icon: '🌳' }
                ].map(opt => (
                  <button
                    key={opt.id}
                    onClick={() => handleAnswer('exp', opt.id)}
                    className="p-4 border-2 border-stone-100 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition text-left"
                  >
          ...
          {quizStep === 2 && (
            <div className="animate-in slide-in-from-right duration-500">
              <span className="text-[10px] uppercase tracking-widest text-amber-600 font-bold mb-2 block">Question 2/4</span>
              <h3 className="text-lg font-bold mb-6">De quel espace disposez-vous pour la cage ?</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { id: 'petit', label: 'Petit (Posé sur un meuble)', desc: 'Max 60-80cm de large' },
                  { id: 'grand', label: 'Grand (Espace dédié)', desc: 'Plus de 100cm ou terrarium' }
                ].map(opt => (
                  <button
                    key={opt.id}
          ...
          {quizStep === 3 && (
            <div className="animate-in slide-in-from-right duration-500">
              <span className="text-[10px] uppercase tracking-widest text-amber-600 font-bold mb-2 block">Question 3/4</span>
              <h3 className="text-lg font-bold mb-6">Que recherchez-vous chez lui ?</h3>
              <div className="grid grid-cols-1 gap-4">
                {[
                  { id: 'docile', label: 'Calme & Docile', desc: 'Facile à prendre en main par des enfants.' },
                  { id: 'affectueux', label: 'Interactif & Affectueux', desc: 'Qui reconnaît son propriétaire et interagit.' },
                  { id: 'vif', label: 'Actif & Fascinant', desc: 'Pour le plaisir de l\'observation de ses acrobaties.' }
                ].map(opt => (
                  <button
                    key={opt.id}
                    onClick={() => handleAnswer('char', opt.id)}
                    className="p-4 border-2 border-stone-100 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition text-left flex items-center justify-between"
                  >
                    <div>
                      <span className="font-bold block">{opt.label}</span>
                      <span className="text-xs text-gray-500">{opt.desc}</span>
          ...
          {quizStep === 4 && (
            <div className="animate-in slide-in-from-right duration-500">
              <span className="text-[10px] uppercase tracking-widest text-amber-600 font-bold mb-2 block">Question 4/4</span>
              <h3 className="text-lg font-bold mb-6">Pour qui est cet animal ?</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { id: 'enfant', label: 'Un Enfant', icon: '👶' },
                  { id: 'famille', label: 'Toute la Famille', icon: '👨‍👩‍👧‍👦' },
                  { id: 'adulte', label: 'Un Adulte', icon: '🧑' },
                  { id: 'passionne', label: 'Un Passionné', icon: '🔎' }
                ].map(opt => (
                  <button
                    key={opt.id}
                    onClick={() => handleAnswer('who', opt.id)}
                    className="p-4 border-2 border-stone-100 rounded-xl hover:border-amber-500 hover:bg-amber-50 transition text-left"
                  >
                    <span className="text-2xl mb-2 block">{opt.icon}</span>
                    <span className="font-bold">{opt.label}</span>
          ...
          {quizStep === 5 && (
            <div className="animate-in fade-in slide-in-from-bottom duration-700 text-center">
              <span className="bg-green-100 text-green-700 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter mb-4 inline-block">Résultat optimal</span>
              <h3 className="text-2xl font-black mb-6">Le {speciesData[getResult()].name}</h3>

              <div className="relative mb-6 rounded-2xl overflow-hidden shadow-2xl group max-w-sm mx-auto">
                <img
                  src={speciesData[getResult()].image}
                  alt={speciesData[getResult()].name}
                  className="w-full aspect-square object-cover"
                />
                <div className="absolute top-4 right-4 flex gap-2">
                  {speciesData[getResult()].tags.map(tag => (
                    <span key={tag} className="bg-amber-500/90 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded-md">{tag}</span>
                  ))}
                </div>
              </div>

              <p className="text-gray-700 mb-8 leading-relaxed">
                {speciesData[getResult()].desc}
              </p>

              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  onClick={resetQuiz}
                  className="px-6 py-3 border-2 border-stone-100 rounded-xl font-bold hover:bg-stone-50 transition text-sm"
                >
                  Recommencer le test
                </button>
                {getResult() === 'russe' && (
                  <a
                    href="#contact"
                    className="px-6 py-3 bg-amber-600 text-white rounded-xl font-bold hover:bg-amber-700 transition shadow-lg text-sm"
                  >
                    Réserver un bébé ({speciesData[getResult()].name})
                  </a>
                )}
              </div>
            </div>
      ...
      <div className="space-y-6 text-gray-700 bg-white p-6 rounded-2xl border border-amber-100 shadow-inner">
        ...
        {quizStep === 5 && (
          <div className="animate-in fade-in slide-in-from-bottom duration-700 text-center">
            ...
            <div className="relative mb-6 rounded-2xl overflow-hidden shadow-2xl group max-w-sm mx-auto">
              <img
                src={getResult() === 'russe' ? '/hamster_russe_result_1767561339558.png' : (getResult() === 'syrien' ? '/hamster_syrien_result2_1767561359036.png' : speciesData[getResult()].image)}
                alt={speciesData[getResult()].name}
                className="w-full aspect-square object-cover"
              />
              <div className="absolute top-4 right-4 flex gap-2">
